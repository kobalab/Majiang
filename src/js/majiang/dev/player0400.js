...
eval_shoupai(shoupai, paishu, dapai) {
    ...
    if (! paishu) {                                                 // for DEBUG
        ...
        paishu = this._suanpai.suan_paishu();                       // for DEBUG
        console.table(this._suanpai._paishu);                       // for DEBUG
        console.table(this._suanpai.paijia_all());                  // for DEBUG
    }                                                               // for DEBUG
                                                                    // for DEBUG
    let rv;
    let n_xiangting = Majiang.Util.xiangting(shoupai);
    ...
    if (n_xiangting == -1) {
        ...
        for (let p of shoupai.get_dapai()) {
            ...
            if (debug) console.log(p, r);                           // for DEBUG
            if (r > max) max = r;
        }
        rv = max;
    }
        ...
        for (let p of add_hongpai(Majiang.Util.tingpai(shoupai))) {
            ...
            paishu[p]++;
            r += ev * paishu[p];
            if (debug) console.log(p, paishu[p], ev);               // for DEBUG
        }
    ...
    if (debug) console.log(paistr, n_xiangting, rv);                // for DEBUG
    return rv;
}
